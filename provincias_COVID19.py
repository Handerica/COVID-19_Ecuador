...
df_COV_EC = pd.read_csv('Base_covid19_msp_23abr2020.csv', sep=';', delimiter=None, header='infer', encoding='ISO-8859-1')
...
df_COV_EC.replace(d, inplace=True)

# 'cod_cant_res', 'cant_residencia'
se_provs = df_COV_EC[df_COV_EC['resultado_laboratorio']=='Positivo'].groupby(['prov_residencia']).count().loc[:, 'total_muestras']
...
for lat, lng, label in zip(df_loc_provs.lat, df_loc_provs.lng, df_loc_provs.admin):
	for prov, casos in zip(df_provs.prov_residencia, df_provs.total_muestras):
		if prov == label.upper():
			...
			folium.Marker([lat,lng], popup=cad, color='red').add_to(ec_map)
