...
# Create map object and locate map in Ecuador
ec_map = folium.Map(location=[-1.831239, -78.183403], zoom_start=7, tiles='cartodbpositron') 


df_COVID19_EC = pd.read_csv('Base_covid19_msp_23abr2020.csv', sep=';', header='infer', encoding='ISO-8859-1')

# Pre-processing data
se_provs = df_COVID19_EC[df_COVID19_EC['clasificacion_caso']=='Confirmado'].groupby(['prov_residencia']).count().loc[:, 'total_muestras']
...
# Iteration to create a marker for each province
